...
class uploader(object):
    ...
    def setParams(self, args):
        timeout = 3 # timeout in Seconds for each command
        setParams = {   'S1':int(args.setBaudrate/1000),    # SERIAL_SPEED
                        'S3':args.setNetId,                 # NETID
                        'S14':int(args.setRTSCTS),          # RTSCTS
                        'S18':args.setNodeCount,            # NODECOUNT
                        'S15':args.setNodeId,               # NODEID
                        }
        ...
        if self.syncAT():
            ...
            time.sleep(0.2)
            self.port.flushInput()
            for param in setParams.iteritems():
                start = time.time()
                self.send('AT%s=%d\r\n'%param, True)
                if not self.expect('OK', timeout=0.5): 
                    time.sleep(0.2)
                    paramsOk = False
...
if __name__ == '__main__':
    ...
    parser.add_argument("--setParams", action='store_true', default=False, help='set parameters after uploading firmware')
    parser.add_argument("--onlyParams", action='store_true', default=False, help='set parameters without uploading')
    parser.add_argument("--setBaudrate", type=int, default=57600, help='set baud rate')
    parser.add_argument("--setNetId", type=int, default=12, help='network id')
    parser.add_argument("--setRTSCTS", action='store_true', default=False, help='Hardware flow controll')
    parser.add_argument("--setNodeCount", type=int, default=2, help='total nodes in a network')
    parser.add_argument("--setNodeId", type=int, default=1, help='node id')
